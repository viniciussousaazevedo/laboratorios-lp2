...
public class Jogo {
	...
	public void rodada() throws EstrategiaInvalidaException, JogadaInvalidaException {
		...
		if (jogador1.getNumPecas() == 0) {
			...
			incrementaPontos(this.jogador1);
			return;
		}
		...
		if (jogador2.getNumPecas() == 0) {
			...
			incrementaPontos(this.jogador2);
			return;
		}

		// se ambos passaram, é um empate (que possui critérios para um possível desempate)
	...
	/**
	 * desempata o jogo no caso dos dois jogadores não terem mais peças para jogar na mesa.
	 * Se um dos jogadores tiver menos peças na mão que o outro, ele será vencedor.
	 * Caso eles tenham o mesmo número de peças na mão, o jogador que possuir 
	 * menor soma de números das peças será o vencedor.
	 * 
	 * O jogo só deve ser empate se ambos tiverem a mesma qu.
	 */
	private void desempate() {
		if (this.jogador1.getNumPecas() < this.jogador2.getNumPecas()) {
			// J1 venceu por menor quantidade de peças
			this.finalizado = true;
			this.vencedor = this.jogador1.getNome();
			incrementaPontos(this.jogador1);
			
		} else if (this.jogador2.getNumPecas() < this.jogador1.getNumPecas()) {
			// J2 venceu por menor quantidade de peças
			this.finalizado = true;
			this.vencedor = this.jogador2.getNome();
			incrementaPontos(this.jogador2);
			...
			int somaPecasJ1 = 0;
			int somaPecasJ2 = 0;
			for (int i = 0; i < this.jogador1.getMao().size(); i++) {
				somaPecasJ1 += this.jogador1.getMao().get(i).getNumEsquerdo() + this.jogador1.getMao().get(i).getNumDireito();
				somaPecasJ2 += this.jogador2.getMao().get(i).getNumEsquerdo() + this.jogador2.getMao().get(i).getNumDireito();
			}
			
			if (somaPecasJ1 < somaPecasJ2) {
				// J1 venceu por menor soma de peças
				this.finalizado = true;
				this.vencedor = this.jogador1.getNome();
				incrementaPontos(this.jogador1);
			
			} else if (somaPecasJ2 < somaPecasJ1) {
				// J2 venceu por menor soma de peças
				this.finalizado = true;
				this.vencedor = this.jogador2.getNome();
				incrementaPontos(this.jogador2);
	...
	/**
	 * incrementa a quantidade de pontos do jogador vencedor. Serve para caso o jogo seja jogado várias vezes seguidas.
	 * Caso o jogador bata com uma peça com lados iguais e que encaixa no outro lado da mesa (jogada chamada "quadrada"), ganha 6 pontos.
	 * Caso o jogador apenas bata com uma peça que encaixa no outro lado da mesa (jogada chamada "lá e ló"), ganha 3 pontos.
	 * Caso o jogador apenas bata com uma peça com lados iguais (jogada chamada "carroça"), ganha 2 pontos.
	 * Caso o jogador vença sem bater (vence por critérios de desempate), ganha 1 ponto.
	 * Caso o jogador vença, mas sem encaixar em nenhuma dessas situações (jogada chamada "batida normal"), ganha 1 ponto.
	 * 
	 * @param vencedor O jogador vencedor
	 */
	private void incrementaPontos(Jogador vencedor) {
		Jogada ultimaJogada = vencedor.getUltimaJogada();
		Peca ultimaPeca = ultimaJogada.getPeca();
		
		vencedor.incrementaPontos(1);
		
		if (vencedor.getNumPecas() == 0) {
			if (ultimaPeca.getNumEsquerdo() == ultimaPeca.getNumDireito()) {
				vencedor.incrementaPontos(1);
				if (ultimaPeca.encaixa(this.mesa.getNumNaEsquerda()) && ultimaPeca.encaixa(this.mesa.getNumNaDireita())) {
					vencedor.incrementaPontos(4);
				}
			} else {
				if (ultimaPeca.encaixa(this.mesa.getNumNaEsquerda()) && ultimaPeca.encaixa(this.mesa.getNumNaDireita())) {
					vencedor.incrementaPontos(2);
				}
			}
		}
	}
